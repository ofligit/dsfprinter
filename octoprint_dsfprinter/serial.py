...
class Serial(object):
	...
	def __init__(self, printer: Printer, settings, seriallog_handler=None,
		...
		self._dsfSd = self._settings.global_get_basefolder("dsfSd")
		...
		bufferThread = threading.Thread(target=self._processBuffer, name="octoprint.plugins.dsfprinter.buffer_thread")
		bufferThread.start()
	...
	@property
	def timeout(self):
		self._logger.debug("timeout -> {}s".format(self._read_timeout))
		return self._read_timeout

	@timeout.setter
	def timeout(self, value):
		self._logger.debug("timeout({}s)".format(value))
		self._read_timeout = value

	@property
	def write_timeout(self):
		self._logger.debug("write_timeout -> {}s".format(self._write_timeout))
		return self._write_timeout

	@write_timeout.setter
	def write_timeout(self, value):
		self._logger.debug("write_timeout({}s)".format(value))
		self._write_timeout = value

	@property
	def port(self):
		self._logger.debug("port -> {}s".format("DSF"))
		return "DSF"

	@property
	def baudrate(self):
		self._logger.debug("baudrate -> {}s".format(self._faked_baudrate))
		return self._faked_baudrate

	# noinspection PyMethodMayBeStatic
	def _clearQueue(self, q):
		self._logger.debug("_clearQueue({}s)".format(q))
		try:
	...
	def _processIncoming(self):
		self._logger.debug("_processIncoming()")
		...
		while self.incoming is not None and not self._killed:
			...
			if data.strip() == "version":
				...
				self._send("Duet Software Framework Printer v" + __version__)
				continue
	...
	def _gcode_T(self, code, data):
		# type: (str, str) -> None
		self._logger.debug("_gcode_T(code={}s, data={}s)".format(code, data))
		t = int(code)
	...
	def _gcode_F(self, code, data):
		# type: (str, str) -> bool
		self._logger.debug("_gcode_F(code={}s, data={}s)".format(code, data))
		if self._supportF:
	...
	def _gcode_M114(self, data):
		# type: (str) -> bool
		self._logger.debug("_gcode_M114(data={}s)".format(data))
		...
		e["all"] = " ".join(
			["E{}:{}".format(num, self._lastE[self.currentExtruder]) for num in range(self.extruderCount)])
	...
	def _gcode_M117(self, data):
		# type: (str) -> None
		self._logger.debug("_gcode_M117(data={}s)".format(data))
	...
	def _gcode_M155(self, data):
		# type: (str) -> None
		self._logger.debug("_gcode_M155(data={}s)".format(data))
		matchS = re.search(r"S([0-9]+)", data)
		if matchS is not None:
			interval = int(matchS.group(1))
			if self._temperature_reporter is not None:
			...
			if interval > 0:
				self._temperature_reporter = RepeatedTimer(interval,
														   lambda: self._send(self._generateTemperatureOutput()))
	...
	def _gcode_G28(self, data):
		# type: (str) -> None
		self._logger.debug("_gcode_G28(data={}s)".format(data))
		self._home(data)
	...
	def _listSd(self):
		...
		if self._settings.get_boolean(["extendedSdFileList"]):
			items = map(
				lambda x: "%s %d" % (x.upper(), os.stat(os.path.join(self._dsfSd, x)).st_size),
				os.listdir(self._dsfSd)
			)
		else:
			items = map(
				lambda x: x.upper(),
	...
	def _selectSdFile(self, filename, check_already_open=False):
		...
		file = os.path.join(self._dsfSd, filename.lower())
	...
	def write(self, data):
		...
		with self._incoming_lock:
			...
			try:
				written = int(self.incoming.put(data, timeout=self._write_timeout, partial=True))
				self._seriallog.info("<<< {}".format(u_data))
				return written
			except queue.Full:
...
class CharCountingQueue(queue.Queue):
	...
	def put(self, item, block=True, timeout=None, partial=False):
		self._logger.debug("put(item={}s, block={}s, timeout={}s, partial={}s)".format(item, block, timeout, partial))
	...
	# noinspection PyMethodMayBeStatic
	def _len(self, item):
		self._logger.debug("len({}s) -> {}s".format(item, len(item)))
		return len(item)
	...
	def _put(self, item):
		self._logger.debug("_put(item={}s)".format(item))
	...
	def _get(self):
		...
		self._size -= self._len(item)
		self._logger.debug("_get() -> {}s".format(item))
		return item

	def _will_it_fit(self, item):
		fit = self.maxsize - self._qsize() >= self._len(item)
		self._logger.debug("_will_it_fit(item={}s) -> {}s".format(item, fit))
		return fit
